...
// fleet list. Each ski links to /ski/[skiId]?owner=<athleteUid> so
// the ski-detail page reads from the athlete's tree (and hides the
// Edit / Retire controls). Firestore rules permit a linked coach to
// read users/{athleteUid}/skis/*.
    ...
    <div>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        <ul className="space-y-3">
          {skis.map(ski => (
            <li key={ski.id}>
              <Link href={`/ski/${ski.id}?owner=${athleteId}`}>
