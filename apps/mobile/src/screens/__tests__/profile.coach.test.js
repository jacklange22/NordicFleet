...
describe('ProfileScreen — add/change/remove coach', () => {
  ...
  it('valid coach email writes coachId on the athlete profile', async () => {
    authMock.__setCurrentUser({uid: 'a1', email: 'a@b.com'});
    firestoreMock.__seedDoc('users/a1', {
      email: 'a@b.com',
      role: 'athlete',
    ...
    // Athlete's coachId now points to the coach uid.
    const stored = firestoreMock.__getStore().get('users/a1');
    expect(stored.coachId).toBe('c1');
  });
  ...
  it('coach lookup miss shows the clear inline error', async () => {
    ...
    // No coach seeded → setCoachByEmail's findCoachByEmail returns null →
  ...
  it('with linked coach, "Remove coach" clears the coachId', async () => {
    authMock.__setCurrentUser({uid: 'a1', email: 'a@b.com'});
    seedCoach('c1');
    firestoreMock.__seedDoc('users/a1', {
      email: 'a@b.com',
